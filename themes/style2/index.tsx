...
import { ScrollToTopButton, StickyShareBar } from '@/components/shared/ArticleEnhancements'
        ...
        {items.slice(0, 5).map((a, idx) => (
          ...
          >
            <span className={`flex-shrink-0 w-7 h-7 flex items-center justify-center text-sm font-black rounded-sm ${
              idx === 0 ? 'bg-[hsl(var(--primary))] text-white' :
              idx === 1 ? 'bg-zinc-700 text-white' :
              'bg-zinc-100 text-zinc-500'
            }`}>{idx + 1}</span>
            <div className="flex-1 min-w-0">
              <span className="text-sm font-semibold text-zinc-900 line-clamp-2 group-hover:text-[hsl(var(--primary))] transition-colors leading-snug">
                {a.title}
              </span>
              {a.publishedAt ? (
            ...
            {a.coverImage?.url ? (
              <div className="flex-shrink-0 w-14 h-10 overflow-hidden bg-zinc-100 rounded-sm">
                {/* eslint-disable-next-line @next/next/no-img-element */}
                <img src={a.coverImage.url} alt={a.title} className="w-full h-full object-cover group-hover:scale-105 transition-transform" loading="lazy" />
              </div>
            ) : null}
          </Link>
        ))}
  ...
  return (
    <div className="bg-white border border-zinc-200 overflow-hidden">
      <div className="bg-blue-600 px-4 py-3 flex items-center gap-2">
        ...
        <h3 className="text-sm font-bold text-white uppercase tracking-wider">తాజా వార్తలు</h3>
      </div>
      <div className="divide-y divide-zinc-100">
        {items.slice(0, 6).map((a) => (
          ...
          >
            {a.coverImage?.url ? (
              <div className="flex-shrink-0 w-12 h-9 overflow-hidden bg-zinc-100 rounded-sm">
                {/* eslint-disable-next-line @next/next/no-img-element */}
                <img src={a.coverImage.url} alt={a.title} className="w-full h-full object-cover" loading="lazy" />
              </div>
            ) : null}
            <div className="flex-1 min-w-0">
              <span className="text-sm font-medium text-zinc-900 line-clamp-2 group-hover:text-blue-600 transition-colors leading-snug">
                {a.title}
              </span>
              {a.publishedAt ? (
                <time className="mt-0.5 block text-xs text-zinc-400">
...
export async function ThemeArticle({ 
  ...
  const visibleFaqItems = buildFaqFromArticle(article, 'te', {
    skipSummaryQuestion: skipSummaryFaq,
    skipHighlightsInFaq: skipHighlightsFaq,
  })

  const categorySlug = getCategorySlugFromArticle(article)
  const rawCatName = article.category?.name || article.categories?.[0]?.name
  const categoryName = typeof rawCatName === 'string' ? rawCatName : 'వార్త'
  const articleUrl = `https://${tenantDomain || 'kaburlutoday.com'}${safeArticleHref(tenantSlug, article, categorySlug)}`

  return (
    <div className="theme-style2 pb-20 sm:pb-0" style={cssVars}>
      ...
      <main id="main-content" className="mx-auto max-w-7xl px-4 py-6 sm:py-8">
        <div className="grid grid-cols-1 gap-8 lg:grid-cols-[minmax(0,1fr)_320px]">
          <article className="min-w-0">
            <nav className="mb-4 hidden sm:flex items-center gap-2 text-sm text-zinc-500" aria-label="Breadcrumb">
              <Link href={toHref(homeHref(tenantSlug))} className="hover:text-[hsl(var(--primary))] transition-colors">హోమ్</Link>
              {categorySlug ? (
                <>
                  <span aria-hidden>›</span>
                  <Link href={toHref(categoryHref(tenantSlug, categorySlug))} className="hover:text-[hsl(var(--primary))] transition-colors">
                    {String(categoryName)}
                  </Link>
                </>
              ) : null}
              <span aria-hidden>›</span>
              <span className="max-w-xs truncate font-medium text-zinc-800" title={article.title}>
                {article.title.length > 48 ? `${article.title.substring(0, 48)}…` : article.title}
              </span>
            </nav>
            
            <header className="mb-6">
              <h1 className="s2-article-page-title mb-4 text-2xl sm:text-3xl lg:text-4xl font-bold leading-tight tracking-tight text-black">
                {article.title}
              </h1>
            ...
            <div className="s2-article-actions mb-6 overflow-x-auto">
      ...
      <StickyShareBar url={articleUrl} title={article.title} scrollThreshold={320} />
      <ScrollToTopButton />
