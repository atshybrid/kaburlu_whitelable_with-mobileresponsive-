...
export default function MobileBottomNavClient({ homeHref, latestHref, categories }: Props) {
  ...
  return (
    <div className="sm:hidden">
      {/* Bottom bar */}
      <nav className="fixed inset-x-0 bottom-0 z-40 border-t border-zinc-200 bg-white/95 pb-[env(safe-area-inset-bottom)] backdrop-blur-lg shadow-[0_-4px_20px_rgba(0,0,0,0.08)]">
        <div className="mx-auto flex max-w-7xl items-stretch justify-between">
          <a href={homeHref} className="flex flex-1 flex-col items-center justify-center gap-1 py-3 text-xs font-medium text-zinc-600 transition-colors hover:bg-red-50 hover:text-red-600">
            <svg width="22" height="22" viewBox="0 0 24 24" fill="currentColor"><path d="M12 3 3 10h2v10h6v-6h2v6h6V10h2L12 3Z"/></svg>
            <span>Home</span>
          </a>
          <a href={latestHref || homeHref} className="flex flex-1 flex-col items-center justify-center gap-1 py-3 text-xs font-medium text-zinc-600 transition-colors hover:bg-red-50 hover:text-red-600">
          ...
          <button type="button" onClick={() => setOpen(true)} className="flex flex-1 flex-col items-center justify-center gap-1 py-3 text-xs font-medium text-zinc-600 transition-colors hover:bg-red-50 hover:text-red-600">
            <svg width="22" height="22" viewBox="0 0 24 24" fill="currentColor"><path d="M3 18h18v-2H3v2zm0-5h18v-2H3v2zm0-7v2h18V6H3z"/></svg>
            <span>Categories</span>
          </button>
          <a href="#top" className="flex flex-1 flex-col items-center justify-center gap-1 py-3 text-xs font-medium text-zinc-600 transition-colors hover:bg-red-50 hover:text-red-600">
            ...
            <div className="grid max-h-[65vh] grid-cols-2 gap-2 overflow-auto p-4">
              {categories.map((c) => (
                <a 
                  key={c.href} 
                  href={c.href} 
                  className="flex items-center gap-2 rounded-xl border border-zinc-200 bg-zinc-50 px-4 py-3 text-sm font-medium text-zinc-700 transition-all hover:border-red-200 hover:bg-red-50 hover:text-red-700"
                >
