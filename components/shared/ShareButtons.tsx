  ...
  const share = async (platform: string) => {
    ...
    try {
      if (platform === 'native' && typeof navigator !== 'undefined' && 'share' in navigator) {
        ...
        window.open(`https://twitter.com/intent/tweet?url=${encodeURIComponent(fullUrl)}&text=${encodeURIComponent(title)}`, '_blank', 'width=600,height=400')
      ...
      <div className="flex gap-2">
        ...
        <button
          ...
          className={`inline-flex items-center justify-center w-9 h-9 rounded-full transition-colors ${
            copied ? 'bg-green-600 text-white' : 'bg-zinc-700 text-white hover:bg-zinc-800'
          }`}
